...
def define_df_clean(dict_data_key_raw):
    ...
    elif dict_data_key_raw == 'data_example':
        df = hdf.get('/df_small')
...
def data_cleaning(df):
    ...
    # clean_example = ( {'train':train, 'test':test} )

    return {'clean':clean, 'prep':prep}

def main():
    dict_data_key_raw = 'data_full'
    df = define_df_clean(dict_data_key_raw)
    clean = data_cleaning(df)['clean']
    prep = data_cleaning(df)['prep']

    if dict_data_key_raw == 'data_example':
        example_status = True
        file_out_clean = 'data_example_clean.h5'
        file_out_prep = 'data_example_prep.h5'
    elif dict_data_key_raw == 'data_full':
        example_status = False
        file_out_clean = 'data_full_clean.h5'
        file_out_prep = 'data_full_prep.h5'
    ...
    directory = os.path.dirname(os.path.abspath(file_out_clean))
    directory
    clean_filename = os.path.join(directory,'data/processed/',file_out_clean)
    clean_filename
    prep_filename = os.path.join(directory, 'data/preprocessed',file_out_prep)
    prep_filename
