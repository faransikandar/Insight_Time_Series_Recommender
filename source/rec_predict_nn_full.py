...
df_nn_full = pd.concat([df_reference, baseline], axis = 1)
df_nn_full.shape
df_nn_full
df_nn_full.describe()
...
df_recs = df_nn_full.loc[(df_nn_full['location_id'] == location) & (df_nn_full['year'] == 2014)]
...
df_nn_full[df_nn_full['export_period_train'] == 0].shape

# Redefine the recommendation df
df_recs_new = ( df_nn_full.loc[(df_nn_full['location_id'] == location) & (df_nn_full['year'] == 2014) &
                (df_nn_full['export_period_train'] == 0)] )
...
print(cosine_similarity([df_nn_full['export_period_test']], [df_nn_full['predictions']]))
print(cosine_similarity([df_nn_full['export_pct_change']], [df_nn_full['pred_pct_change']]))

print(mean_absolute_error([df_nn_full['export_pct_change']], [df_nn_full['pred_pct_change']]))
print(mean_squared_error([df_nn_full['export_pct_change']], [df_nn_full['pred_pct_change']]))

#%%
# Next step is to rank the pct_change actual vs. predict and do MSE validation
df_nn_full['export_pct_change_rank'] = df_nn_full['export_pct_change'].rank(ascending=False, method='min')
df_nn_full.sort_values(by=['export_pct_change_rank'])

# REALLY useful - shows the top predicted export pct change areas - travel/tourism, computer inputs, etc. dominate
df_nn_full['pred_pct_change_rank'] = df_nn_full['pred_pct_change'].rank(ascending=False, method='min')
df_nn_full.sort_values(by=['pred_pct_change_rank'])


#%%
# Calculate MSE between export_pct and pred_pct ranks
print(cosine_similarity([df_nn_full['export_pct_change_rank']], [df_nn_full['pred_pct_change_rank']]))
print(mean_absolute_error([df_nn_full['export_pct_change_rank']], [df_nn_full['pred_pct_change_rank']]))
print(mean_squared_error([df_nn_full['export_pct_change_rank']], [df_nn_full['pred_pct_change_rank']]))

# See if there's much difference between train and test export values
df_nn_full['export_period_train_rank'] = df_nn_full['export_period_train'].rank(ascending=False, method='min')
df_nn_full['export_period_test_rank'] = df_nn_full['export_period_test'].rank(ascending=False, method='min')
print(cosine_similarity([df_nn_full['export_period_train_rank']], [df_nn_full['export_period_test_rank']]))

#%%
# Save df to hdf5
df_nn_full.to_hdf('data/processed/data_predictions.h5', key='df_nn_full2')
